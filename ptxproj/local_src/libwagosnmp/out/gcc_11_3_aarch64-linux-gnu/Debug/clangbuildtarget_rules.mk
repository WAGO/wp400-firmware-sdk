...
$(DEPS_libwagosnmp.so) $(OBJS_libwagosnmp.so) $(LOBS_libwagosnmp.so) $(TIDYS_libwagosnmp.so) $(PLINTS_libwagosnmp.so) ./out/gcc_11_3_aarch64-linux-gnu/Debug/libwagosnmp.so/lint_mac.h: BUILDTARGET_CPPFLAGS:=$(libwagosnmp.so_CPPFLAGS)

$(DEPS_libwagosnmp.so) $(OBJS_libwagosnmp.so) $(TIDYS_libwagosnmp.so) $(PLINTS_libwagosnmp.so) ./out/gcc_11_3_aarch64-linux-gnu/Debug/libwagosnmp.so/lint_mac.h: BUILDTARGET_CXXFLAGS:=$(libwagosnmp.so_CXXFLAGS)

$(DEPS_libwagosnmp.so) $(OBJS_libwagosnmp.so) $(TIDYS_libwagosnmp.so) $(PLINTS_libwagosnmp.so) ./out/gcc_11_3_aarch64-linux-gnu/Debug/libwagosnmp.so/lint_mac.h: BUILDTARGET_CFLAGS:=$(libwagosnmp.so_CFLAGS)
...
./out/gcc_11_3_aarch64-linux-gnu/Debug/info/libwagosnmp.so-info.txt: $(SCRIPT_DIR)/internal/clang/info_txt.template $(MK_FILES); $(SILENT)$(call clangbuild_subst_info_buildtarget,libwagosnmp.so,$<) > $@
...
$(OUT_DIR)/libwagosnmp.so/%.tidy: $(PROJECT_ROOT)/% $(MK_FILES) $(LINT_SUPPORT_FILES) $(libwagosnmp.so_PRECLANG_FILES) ./out/gcc_11_3_aarch64-linux-gnu/Debug/libwagosnmp.so/%.o.d $(wildcard .clang-tidy) | $(TIDY_SUPPORT_DIRS); $(SILENT)$(call clang-tidy,$@,$<,$(FLAGS))

$(OUT_DIR)/libwagosnmp.so/%.plint: $(PROJECT_ROOT)/% $(MK_FILES) $(LINT_SUPPORT_FILES) $(libwagosnmp.so_PRECLANG_FILES) ./out/gcc_11_3_aarch64-linux-gnu/Debug/libwagosnmp.so/%.o.d; $(SILENT)$(call clang-sa,$@,$<,$(FLAGS))

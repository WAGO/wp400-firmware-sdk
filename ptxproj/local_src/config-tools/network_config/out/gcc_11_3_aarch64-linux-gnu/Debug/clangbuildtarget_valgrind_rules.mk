...
nulgrind-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(network_config.elf_VALGRIND_FLAGS)        $(NULGRIND_FLAGS)        $(network_config.elf_NULGRIND_FLAGS) $(call valgrind_xml_flags,network_config.elf,NULGRIND)
nulgrind-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(network_config.elf_VALGRIND_TARGET_FLAGS) $(NULGRIND_TARGET_FLAGS) $(network_config.elf_NULGRIND_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
nulgrind-target-network_config.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
nulgrind-target-network_config.elf: VALGRIND_TARGET_EXECUTABLE=network_config.elf
nulgrind-target-network_config.elf: VG_TOOL=nulgrind
nulgrind-target-network_config.elf: $(BIN_DIR)/network_config.elf $(VALGRIND)
nulgrind-target-network_config.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
nulgrind-target-network_config.elf:
	$(call vg-version)
	$(call vg-caller)
...
memcheck-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(network_config.elf_VALGRIND_FLAGS)        $(MEMCHECK_FLAGS)        $(network_config.elf_MEMCHECK_FLAGS) $(call valgrind_xml_flags,network_config.elf,MEMCHECK)
memcheck-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(network_config.elf_VALGRIND_TARGET_FLAGS) $(MEMCHECK_TARGET_FLAGS) $(network_config.elf_MEMCHECK_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
memcheck-target-network_config.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
memcheck-target-network_config.elf: VALGRIND_TARGET_EXECUTABLE=network_config.elf
memcheck-target-network_config.elf: VG_TOOL=memcheck
memcheck-target-network_config.elf: $(BIN_DIR)/network_config.elf $(VALGRIND)
memcheck-target-network_config.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
memcheck-target-network_config.elf:
	$(call vg-version)
	$(call vg-caller)
...
helgrind-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(network_config.elf_VALGRIND_FLAGS)        $(HELGRIND_FLAGS)        $(network_config.elf_HELGRIND_FLAGS) $(call valgrind_xml_flags,network_config.elf,HELGRIND)
helgrind-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(network_config.elf_VALGRIND_TARGET_FLAGS) $(HELGRIND_TARGET_FLAGS) $(network_config.elf_HELGRIND_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
helgrind-target-network_config.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
helgrind-target-network_config.elf: VALGRIND_TARGET_EXECUTABLE=network_config.elf
helgrind-target-network_config.elf: VG_TOOL=helgrind
helgrind-target-network_config.elf: $(BIN_DIR)/network_config.elf $(VALGRIND)
helgrind-target-network_config.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
helgrind-target-network_config.elf:
	$(call vg-version)
	$(call vg-caller)
...
callgrind-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(network_config.elf_VALGRIND_FLAGS)        $(CALLGRIND_FLAGS)        $(network_config.elf_CALLGRIND_FLAGS) $(call valgrind_xml_flags,network_config.elf,CALLGRIND)
callgrind-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(network_config.elf_VALGRIND_TARGET_FLAGS) $(CALLGRIND_TARGET_FLAGS) $(network_config.elf_CALLGRIND_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
callgrind-target-network_config.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
callgrind-target-network_config.elf: VALGRIND_TARGET_EXECUTABLE=network_config.elf
callgrind-target-network_config.elf: VG_TOOL=callgrind
callgrind-target-network_config.elf: $(BIN_DIR)/network_config.elf $(VALGRIND)
callgrind-target-network_config.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
callgrind-target-network_config.elf:
	$(call vg-version)
	$(call vg-caller)
...
cachegrind-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(network_config.elf_VALGRIND_FLAGS)        $(CACHEGRIND_FLAGS)        $(network_config.elf_CACHEGRIND_FLAGS) $(call valgrind_xml_flags,network_config.elf,CACHEGRIND)
cachegrind-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(network_config.elf_VALGRIND_TARGET_FLAGS) $(CACHEGRIND_TARGET_FLAGS) $(network_config.elf_CACHEGRIND_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
cachegrind-target-network_config.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
cachegrind-target-network_config.elf: VALGRIND_TARGET_EXECUTABLE=network_config.elf
cachegrind-target-network_config.elf: VG_TOOL=cachegrind
cachegrind-target-network_config.elf: $(BIN_DIR)/network_config.elf $(VALGRIND)
cachegrind-target-network_config.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
cachegrind-target-network_config.elf:
	$(call vg-version)
	$(call vg-caller)
...
massif-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(network_config.elf_VALGRIND_FLAGS)        $(MASSIF_FLAGS)        $(network_config.elf_MASSIF_FLAGS) $(call valgrind_xml_flags,network_config.elf,MASSIF)
massif-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(network_config.elf_VALGRIND_TARGET_FLAGS) $(MASSIF_TARGET_FLAGS) $(network_config.elf_MASSIF_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
massif-target-network_config.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
massif-target-network_config.elf: VALGRIND_TARGET_EXECUTABLE=network_config.elf
massif-target-network_config.elf: VG_TOOL=massif
massif-target-network_config.elf: $(BIN_DIR)/network_config.elf $(VALGRIND)
massif-target-network_config.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
massif-target-network_config.elf:
	$(call vg-version)
	$(call vg-caller)
...
drd-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(network_config.elf_VALGRIND_FLAGS)        $(DRD_FLAGS)        $(network_config.elf_DRD_FLAGS) $(call valgrind_xml_flags,network_config.elf,DRD)
drd-target-network_config.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(network_config.elf_VALGRIND_TARGET_FLAGS) $(DRD_TARGET_FLAGS) $(network_config.elf_DRD_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
drd-target-network_config.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
drd-target-network_config.elf: VALGRIND_TARGET_EXECUTABLE=network_config.elf
drd-target-network_config.elf: VG_TOOL=drd
drd-target-network_config.elf: $(BIN_DIR)/network_config.elf $(VALGRIND)
drd-target-network_config.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
drd-target-network_config.elf:
	$(call vg-version)
	$(call vg-caller)

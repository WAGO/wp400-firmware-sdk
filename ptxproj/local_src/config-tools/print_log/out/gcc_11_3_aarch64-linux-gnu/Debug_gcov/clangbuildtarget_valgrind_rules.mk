...
nulgrind-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(print_log.elf_VALGRIND_FLAGS)        $(NULGRIND_FLAGS)        $(print_log.elf_NULGRIND_FLAGS) $(call valgrind_xml_flags,print_log.elf,NULGRIND)
nulgrind-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(print_log.elf_VALGRIND_TARGET_FLAGS) $(NULGRIND_TARGET_FLAGS) $(print_log.elf_NULGRIND_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
nulgrind-target-print_log.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
nulgrind-target-print_log.elf: VALGRIND_TARGET_EXECUTABLE=print_log.elf
nulgrind-target-print_log.elf: VG_TOOL=nulgrind
nulgrind-target-print_log.elf: $(BIN_DIR)/print_log.elf $(VALGRIND)
nulgrind-target-print_log.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
nulgrind-target-print_log.elf:
	$(call vg-version)
	$(call vg-caller)
...
memcheck-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(print_log.elf_VALGRIND_FLAGS)        $(MEMCHECK_FLAGS)        $(print_log.elf_MEMCHECK_FLAGS) $(call valgrind_xml_flags,print_log.elf,MEMCHECK)
memcheck-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(print_log.elf_VALGRIND_TARGET_FLAGS) $(MEMCHECK_TARGET_FLAGS) $(print_log.elf_MEMCHECK_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
memcheck-target-print_log.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
memcheck-target-print_log.elf: VALGRIND_TARGET_EXECUTABLE=print_log.elf
memcheck-target-print_log.elf: VG_TOOL=memcheck
memcheck-target-print_log.elf: $(BIN_DIR)/print_log.elf $(VALGRIND)
memcheck-target-print_log.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
memcheck-target-print_log.elf:
	$(call vg-version)
	$(call vg-caller)
...
helgrind-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(print_log.elf_VALGRIND_FLAGS)        $(HELGRIND_FLAGS)        $(print_log.elf_HELGRIND_FLAGS) $(call valgrind_xml_flags,print_log.elf,HELGRIND)
helgrind-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(print_log.elf_VALGRIND_TARGET_FLAGS) $(HELGRIND_TARGET_FLAGS) $(print_log.elf_HELGRIND_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
helgrind-target-print_log.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
helgrind-target-print_log.elf: VALGRIND_TARGET_EXECUTABLE=print_log.elf
helgrind-target-print_log.elf: VG_TOOL=helgrind
helgrind-target-print_log.elf: $(BIN_DIR)/print_log.elf $(VALGRIND)
helgrind-target-print_log.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
helgrind-target-print_log.elf:
	$(call vg-version)
	$(call vg-caller)
...
callgrind-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(print_log.elf_VALGRIND_FLAGS)        $(CALLGRIND_FLAGS)        $(print_log.elf_CALLGRIND_FLAGS) $(call valgrind_xml_flags,print_log.elf,CALLGRIND)
callgrind-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(print_log.elf_VALGRIND_TARGET_FLAGS) $(CALLGRIND_TARGET_FLAGS) $(print_log.elf_CALLGRIND_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
callgrind-target-print_log.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
callgrind-target-print_log.elf: VALGRIND_TARGET_EXECUTABLE=print_log.elf
callgrind-target-print_log.elf: VG_TOOL=callgrind
callgrind-target-print_log.elf: $(BIN_DIR)/print_log.elf $(VALGRIND)
callgrind-target-print_log.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
callgrind-target-print_log.elf:
	$(call vg-version)
	$(call vg-caller)
...
cachegrind-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(print_log.elf_VALGRIND_FLAGS)        $(CACHEGRIND_FLAGS)        $(print_log.elf_CACHEGRIND_FLAGS) $(call valgrind_xml_flags,print_log.elf,CACHEGRIND)
cachegrind-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(print_log.elf_VALGRIND_TARGET_FLAGS) $(CACHEGRIND_TARGET_FLAGS) $(print_log.elf_CACHEGRIND_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
cachegrind-target-print_log.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
cachegrind-target-print_log.elf: VALGRIND_TARGET_EXECUTABLE=print_log.elf
cachegrind-target-print_log.elf: VG_TOOL=cachegrind
cachegrind-target-print_log.elf: $(BIN_DIR)/print_log.elf $(VALGRIND)
cachegrind-target-print_log.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
cachegrind-target-print_log.elf:
	$(call vg-version)
	$(call vg-caller)
...
massif-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(print_log.elf_VALGRIND_FLAGS)        $(MASSIF_FLAGS)        $(print_log.elf_MASSIF_FLAGS) $(call valgrind_xml_flags,print_log.elf,MASSIF)
massif-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(print_log.elf_VALGRIND_TARGET_FLAGS) $(MASSIF_TARGET_FLAGS) $(print_log.elf_MASSIF_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
massif-target-print_log.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
massif-target-print_log.elf: VALGRIND_TARGET_EXECUTABLE=print_log.elf
massif-target-print_log.elf: VG_TOOL=massif
massif-target-print_log.elf: $(BIN_DIR)/print_log.elf $(VALGRIND)
massif-target-print_log.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
massif-target-print_log.elf:
	$(call vg-version)
	$(call vg-caller)
...
drd-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL = $(VALGRIND_FLAGS)        $(print_log.elf_VALGRIND_FLAGS)        $(DRD_FLAGS)        $(print_log.elf_DRD_FLAGS) $(call valgrind_xml_flags,print_log.elf,DRD)
drd-target-print_log.elf: VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL = $(VALGRIND_TARGET_FLAGS) $(print_log.elf_VALGRIND_TARGET_FLAGS) $(DRD_TARGET_FLAGS) $(print_log.elf_DRD_TARGET_FLAGS) $(VALGRIND_GTEST_FILTER_FLAGS)
drd-target-print_log.elf: VALGRIND_GTEST_FILTER?=*:-*_Target*.*:*.*_Target*
drd-target-print_log.elf: VALGRIND_TARGET_EXECUTABLE=print_log.elf
drd-target-print_log.elf: VG_TOOL=drd
drd-target-print_log.elf: $(BIN_DIR)/print_log.elf $(VALGRIND)
drd-target-print_log.elf: VALGRIND_ARGS=$(VALGRIND_BUILDTARGET_TOOL_FLAGS_CALL) $(VG_ABS_DUT) $(VALGRIND_BUILDTARGET_TOOL_TARGET_FLAGS_CALL)
drd-target-print_log.elf:
	$(call vg-version)
	$(call vg-caller)

...
# set processor specific compiler flags
ifdef PTXCONF_QT5_IMX6_SUPPORT
	QT5_CXXFLAGS+=-O3 -march=armv7-a -mtune=cortex-a9 -mfpu=vfpv3 -mfloat-abi=hard -DLINUX=1
endif
ifdef PTXCONF_QT5_IMX8_SUPPORT
	QT5_CXXFLAGS+=-O3 -march=armv8-a -mcpu=cortex-a53 -DLINUX=1
endif
...
ifndef PTXCONF_ARCH_LP64
	...
	sed -i "s|IMX_FLAGS|$(QT5_CXXFLAGS)|g" "$(QT5_DIR)/qtbase/mkspecs/linux-ptx-g++/qmake.conf"
...
ifdef PTXCONF_QT5_MODULE_QTWEBENGINE
# PTXdist is a Gnu make based build system, the QtWebEngine module uses ninja as build system.
# For parallel builds, make uses a job server protocol to facilitate recursively called make instances to coordinate themselves
# with each other with regard to resource utilisation.
# Plain vanilla ninja does not obey this job server convention and instead uses the existing CPU resources at will.
# On a build server with multiple parallelly running builds this has led to out-of-memory conditions stalling the whole host multiple times.
#
# ptxproj.2013.03.0/configs/wago-wp400/patches/qt-everywhere-opensource-src-5.15.12/0401-ptxdist-qtwebengine-let-ninja-use-the-jobserver.patch
# contains a patch created by Pengutronix which tells CMake to pass down make's variable MAKEFLAGS to ninja. This patch relies on a ninja version
# that is patched itself. Pengutronix introduced that patched ninja in 2022:
# https://git.pengutronix.de/cgit/ptxdist/commit/rules/host-ninja.make?id=09106f3a0b3079179c0470ed9484bfbd122e2459
# The patched version itself is maintained by KitWare: https://github.com/Kitware/ninja/commit/ff5d8028807beff522bd11bbe9e507fbf6b232a6
# As long as this patched version is not available and working within this ptxdist build, we use a simple work around to limit CPU resources.
#
QT5_MAKE_ENV += \
	NINJAJOBS="-j8"
endif
...
ifdef PTXCONF_QT5_MODULE_QTWEBENGINE
	...
	@$(call install_tree, qt5, 0, 0, $(PTXCONF_SYSROOT_TARGET)/usr/share/qt5/translations, /usr/share/qt5/translations)
endif
